...
def Smooth(p1,p2,p3,min_a):
    ...
    if P == 0.:
        ps[1] = p1[1]+(p3[1]-p1[1])*0.33333333333333333333
        ps[0] = p1[0]+(p3[0]-p1[0])*0.33333333333333333333
        pq[1] = p1[1]+(p3[1]-p1[1])*0.66666666666666666667
        pq[0] = p1[0]+(p3[0]-p1[0])*0.66666666666666666667
        return ps,pq
    ...
    while min_a < MIN_ANGLE:  
        if i == 0:     
            if segments[p_len-2] < SQR_SMOOTH_LENGTH and segments[i] < SQR_SMOOTH_LENGTH:
                ...
                #ps,pq = Smooth(points[p_len-2], points[i], points[i+1], min_a)      
                ...
                angles[i] =  math.pi  #angle was sharp but segments were too long, skip it in next iteration     
            #if segments are smaller then threshold
        else:
            if segments[i-1] < SQR_SMOOTH_LENGTH and segments[i] < SQR_SMOOTH_LENGTH:
                #calculation of the two new points, keeping the area
                ps,pq = Smooth(points[i-1], points[i], points[i+1],min_a)    
                #update the point list
